...
| `--no-precompile`                                          | Skip the serial precompile pass the parallel runner performs before discovery (one `--compile-only` subprocess for all spec files). See [Precompile pass](parallel/parallel.md#precompile-pass).                                                                                                                                                                                            |
| `--compile-only`                                           | Load the given spec files, writing their modules' precompilation, and exit without running examples or printing. Exits 0 when every file loaded, 1 on a load error. See [Precompile pass](parallel/parallel.md#precompile-pass).                                                                                                                                                            |
