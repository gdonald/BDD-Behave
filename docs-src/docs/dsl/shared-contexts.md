...
Because the hook lives in the including group, it inherits into nested `context` blocks the same way an inline hook would. See [Hooks](hooks.md) for the full ordering rules.
