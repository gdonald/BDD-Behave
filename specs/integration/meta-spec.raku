...
describe 'META6.json file', {
  it 'exists at the project root', {
    expect($meta-path.f ?? 1 !! 0).to.be(1);
  }

  it 'is non-empty', {
    expect($meta-path.s > 0 ?? 1 !! 0).to.be(1);
  }
}

describe 'META6.json required fields', {
  it 'declares a name', {
    expect(parse-string-field($meta-path, 'name') eq 'BDD::Behave' ?? 1 !! 0).to.be(1);
  }

  it 'declares a version', {
    my $v = parse-string-field($meta-path, 'version');
    expect($v.defined && $v ~~ /^ \d+ '.' \d+ '.' \d+ $/ ?? 1 !! 0).to.be(1);
  }

  it 'declares a description', {
    my $d = parse-string-field($meta-path, 'description');
    expect($d.defined && $d.chars > 0 ?? 1 !! 0).to.be(1);
  }

  it 'declares a perl version', {
    my $p = parse-string-field($meta-path, 'perl');
    expect($p.defined && $p.chars > 0 ?? 1 !! 0).to.be(1);
  }

  it 'declares an authors list', {
    expect(has-key($meta-path, 'authors') ?? 1 !! 0).to.be(1);
  }

  it 'declares a license', {
    my $l = parse-string-field($meta-path, 'license');
    expect($l.defined && $l.chars > 0 ?? 1 !! 0).to.be(1);
  }

  it 'declares a provides section', {
    expect(has-key($meta-path, 'provides') ?? 1 !! 0).to.be(1);
  }

  it 'declares depends, build-depends, and test-depends arrays', {
    expect(has-key($meta-path, 'depends')       ?? 1 !! 0).to.be(1);
    expect(has-key($meta-path, 'build-depends') ?? 1 !! 0).to.be(1);
    expect(has-key($meta-path, 'test-depends')  ?? 1 !! 0).to.be(1);
  }
}
